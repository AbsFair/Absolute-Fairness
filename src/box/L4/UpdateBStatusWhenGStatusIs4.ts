...
export async function updateBStatusWhenGStatusIs4() {
  ////////////////////////////////////////////////////////////////////////////////
  ...
  if (Date.now() / 1000 - onChainData.bData.update_t > onChainData.bCfgs.time_amt) {
    ...
    betAmountOfTheOppo.value =
      ...
      1000000000
    if (
      (onChainData.bData.g_status == 4 && onChainData.bData.seat_idx == 2) ||
      (onChainData.bData.g_status == 5 && onChainData.bData.seat_idx == 1)
    ) {
      displaiedGameResult.value = 'You timed out.'
      bonusesTheUserWined.value = 0
    } else if (
      (onChainData.bData.g_status == 4 && onChainData.bData.seat_idx == 1) ||
      (onChainData.bData.g_status == 5 && onChainData.bData.seat_idx == 2)
    ) {
  ...
  if (
    ...
    // operation page 1
    operationButtonName.value = 'Submit'
    theMostRecentBid.value = getTheMostRecentBid(onChainData.bData, 'from you.')
    theMostRecentLog.value = 'Your opponent challenged.'
    betAmountOfTheUser.value =
      (onChainData.bData.m_amount * (Math.pow(2, onChainData.bData.game_idx) * basicAmount)) /
      1000000000
    betAmountOfTheOppo.value =
    ...
    theMostRecentLog.value = 'You challenged.'
    betAmountOfTheUser.value =
      (onChainData.bData.m_amount * (Math.pow(2, onChainData.bData.game_idx) * basicAmount)) /
      1000000000
    betAmountOfTheOppo.value =
