...
import { tutorial, goBack, upgrade, query } from '@/functions/modules/3-UpgradePage2'
import { userWeb3WalletAddress, upgradePrice } from '@/state'
</script>

<template>
  <div :class="$style.container">
    <div :class="$style.content">
      <div :class="$style.title">Upgrade Page</div>
      <div :class="$style.subtitle">Upgrade Price</div>
      <div :class="$style.text_box">{{ upgradePrice }} SOL</div>
      <div :class="$style.caption" @click="query()">What is it? Who received it?</div>
      <div :class="$style.divider"></div>
      <div :class="$style.subtitle">Address to Be Upgraded</div>
      <div :class="$style.code_box">{{ userWeb3WalletAddress }}</div>
      <div :class="$style.button_1" @click="goBack()">Go Back</div>
      <div :class="$style.button_2" @click="tutorial()">Tutorial</div>
      <div :class="$style.button_3" @click="upgrade()">Upgrade to Supreme</div>
    </div>
  </div>
</template>
...
.container .content .caption:hover {
  text-decoration: underline;
}
